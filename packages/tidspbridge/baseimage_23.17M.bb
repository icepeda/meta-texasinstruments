...
## Getting system files
  ...
  cp -af ${STAGING_BINDIR}/dspbridge/system/utils/* ${S}/system/utils
  cp -af ${STAGING_BINDIR}/dspbridge/system/inst2/* ${S}/system/inst2
  cp -af ${STAGING_BINDIR}/dspbridge/system/avsync/* ${S}/system/avsync
  cp -af ${STAGING_BINDIR}/dspbridge/system/dasf/* ${S}/system/dasf
  cp -af ${STAGING_BINDIR}/dspbridge/system/hal/* ${S}/system/hal
  cp -af ${STAGING_BINDIR}/dspbridge/system/tmon/* ${S}/system/tmon
## Getting vgpop files
  mkdir -p ${S}/video/alg/vgpop
  cp -af ${STAGING_BINDIR}/dspbridge/video/alg/vgpop/* ${S}/video/alg/vgpop
## Getting MasterConfig files
  mkdir -p ${S}/include
  cp -af ${STAGING_INCDIR}/dspbridge/include/* ${S}/include
## Getting SampleRateConverte files
  mkdir -p ${S}/audio/alg/SampleRateConverter
  cp -af ${STAGING_BINDIR}/dspbridge/audio/alg/SampleRateConverter/* ${S}/audio/alg/SampleRateConverter
  sed -e 's%\\%\/%g' makefile > makefile.linux
## Getting the dsp make system
  mkdir -p ${S}/make
  cp -af ${STAGING_BINDIR}/dspbridge/make/* ${S}/make
